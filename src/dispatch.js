...
async function select(date_available, cost) {  //默认cost是某路径路程的花费
...
module.exports = {
    async post(params, qs, body) {
        // 检验
        if (!body.hasOwnProperty("order")) throw {  //这里要写哪些，是开了什么就要写什么么
            code: 400,
        ...
        //选取飞机
        let para = config["db"]["db"]["dispatch"]["parameter"];
        ...
        for (let i = 0; i < drones.length ; i++){
            ...
            let dispatch_path = await pathCol.find({$and: [{terminal: ObjectID(garageFrom[0].center)}, {terminal: ObjectID(final_task[0].lastPlace)}]}).toArray();
            let dispatch_routine = await routineCol.find({_id: ObjectID(dispatch_path[0].routine)}).toArray();
            let temp = select(final_task[0].finish,dispatch_routine[0].cost);
            if (para < temp[0]){
        ...
        let waypoint1 = temp1_.concat(pinpointFrom[0].coordinate);
        ...
        let waypoint2 = temp2_.concat(pinpointTo[0].coordinate);
...
module.exports.post({}, {}, {}).then(console.log);